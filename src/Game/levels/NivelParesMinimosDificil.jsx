//src/Game/levels/NivelParesMinimosDificil.jsx
import React, { useState, useEffect, useContext, useRef } from 'react';
import { useParams, useNavigate, useLocation } from 'react-router-dom';
import styled from 'styled-components';
import { AuthContext } from '../../context/AuthContext';
import { getNivelConfigParesMinimosDificil } from '../data/nivelesConfigParesMinimosDificil';
...
// Main Component
const NivelParesMinimosDificil = () => {
  // Context and Routing
  ...
  useEffect(() => {
    const navigationKey = `authorized_navigation_pares_dificil_${nivel}`;
    ...
    if (previousLevelRef.current && previousLevelRef.current !== nivel) {
      if (!isAuthorized) {
        ...
        if (user) {
          ...
          localStorage.removeItem(`progress_pares-minimos_dificil_${userId}`);
          localStorage.removeItem(`completed_levels_pares-minimos_dificil_${userId}`);
        }
  ...
  const loadProgressFromDatabase = async () => {
    ...
    try {
      ...
      const response = await ninoService.getProgresoEspecifico(user.id, 'pares-minimos', 'dificil');
  ...
  const saveCompletedLevelProgress = async (completedLevel, finalScore) => {
    if (user) {
      ...
      localStorage.setItem(`progress_pares-minimos_dificil_${userId}`, JSON.stringify(generalProgress));
      ...
      const completedLevelsKey = `completed_levels_pares-minimos_dificil_${userId}`;
      let completedLevels = {};
  ...
  const getCorrectInitialScore = (currentLevel) => {
    ...
    if (user) {
      ...
      const savedProgress = localStorage.getItem(`progress_pares-minimos_dificil_${userId}`);
      if (savedProgress) {
  ...
  // Game State - Inicialización mejorada
  ...
  const closeEndGameAlert = () => {
    ...
    if (endGameType === 'success') {
      ...
      const maxLevels = 5; // Modo dificil tiene 5 niveles
      ...
      if (currentLevel >= maxLevels) {
        ...
        sessionStorage.setItem(`authorized_navigation_pares_dificil_${nextLevel}`, 'true');
        navigate(`/nivel/pares-minimos/dificil/${nextLevel}`);
      }
    } else {
  ...
  // FUNCIONES DE JUEGO MODIFICADAS PARA MODO DIFICIL
    ...
    if (isTraining) {
      ...
      await playAudioWithQueue(indicator.audio, () => {
        setTrainingClicks(prev => {
          ...
          // Verificar si los tres indicadores tienen exactamente 5 clicks
  ...
  const saveProgress = () => {
    if (user) {
      ...
      localStorage.setItem(`progress_pares-minimos_dificil_${userId}`, JSON.stringify(progress));
    }
  };
  ...
  const evaluateLevel = (currentCorrectSelections = correctSelections, currentScore = score) => {
    ...
    if (currentCorrectSelections === winCondition.requiredCorrect && currentScore >= requiredFinalScore) {
      ...
      if (currentLevel < maxLevels) {
        ...
        setEndGameMessage(`¡Felicidades! Has completado todos los niveles de la dificultad medio. ¡Eres increíble!`);
      }
  ...
  useEffect(() => {
    const config = getNivelConfigParesMinimosDificil(nivel);
    if (config) {
      ...
      if (user) {
        ...
        if (nivel === '1') {
          ...
          localStorage.setItem(`progress_pares-minimos_dificil_${userId}`, JSON.stringify(freshProgress));
...
// NUEVO: Container para los tres indicadores
...
export default NivelParesMinimosDificil;